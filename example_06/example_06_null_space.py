...
async def main():
    ...
    await full_body_move_group.move_joints_operation(joint_path, velocity_scaling=0.4).plan().execute_async()
    # ... and back
    reversed_joint_path = JointPath(waypoints[0].joint_set, list(reversed(waypoints)))
    await full_body_move_group.move_joints_operation(reversed_joint_path, velocity_scaling=0.4).plan().execute_async()
